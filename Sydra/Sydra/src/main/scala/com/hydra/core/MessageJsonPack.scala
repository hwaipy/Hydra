...
object MessageJsonPack {
  ...
  //
  def pack(value: Map[String, Any]): String = {
    val jsonAst = "89".toJson
//    println(jsonAst)
    "123"
  }
...
class MessageJsonPacker(flatter: (Any, Option[String]) => RemoteObject = (a: Any, None) => throw new IllegalArgumentException(s"Type ${a.getClass} not recognized.")) {
