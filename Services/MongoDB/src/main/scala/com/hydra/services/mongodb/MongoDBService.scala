...
object StorageService extends App {
  lazy val Configuration = {
    ...
    val propertiesFile = new File("MongoDB.properties")
    try {
  ...
  lazy val clientName = Configuration.getProperty("clientName", "MongoDB")
  ...
  println("MongoDB Service online.")

  generateData()

  Source.stdin.getLines.filter(line => line.toLowerCase == "q").next
  println("Stoping MongoDB Service...")
  client.stop
  ...
  def generateData() {
    ...
    while (true) {
      val doc: Document = Document("Category" -> "VirtualDevice", "Device" -> "D1", "power" -> (100 + random.nextDouble()))
      val insertOperation = collection.insertOne(doc)
      insertOperation.subscribe(new Observer[Completed] {
